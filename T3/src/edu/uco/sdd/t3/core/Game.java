...
/**
 * Controls the rules of the game and serves as an interface for Player objects
 * to communicate with the board. Also observes the board and is notified
 * whenever markers are successfully placed so it can check for player victory.
 * 
 * @author Joshua Ford
 * @version 1.0
 */
public class Game implements BoardObserver {
	...
	/**
	 * Represents the two different game modes: Normal and Sudden Death. The
	 * Normal game mode is used for regular tic-tac-toe. The Sudden Death game
	 * mode limits the number of markers that are able to be placed on the
	 * board, after which the markers are slid around the surface of the board.
	 */
	public enum Mode {
		NORMAL, SUDDEN_DEATH
	}
	...
	/**
	 * Represents the state of the current game. Players take turns placing
	 * markers, and a victory check is done whenever markers are successfully
	 * placed. The previous turn is taken into effect to determine the winner of
	 * the game.
	 */
	public enum State {
	 ...
	 * 
	 * @param gameMode
	 *            The game mode to select. Typically "Normal" or "Sudden Death".
	 */
	...
	/**
	 * This method is called whenever a Board object successfully places a
	 * marker. A victory evaluation is done, and if there is a victory, then any
	 * observers of this game object are notified that the game is over.
	 * 
	 * If there is not a victory and all markers are placed on the board, then
	 * the game is in stalemate, and all observers are notified again that a
	 * stalemate has occurred.
	 * 
	 * If none of the other conditions are true, then it is a regular move, and
	 * the game changes state to reflect whose turn it is.
	 * 
	 * @param The
	 *            MoveAction used to place a marker on the board.
	 */
	@Override
	public void onMarkerPlaced(MoveAction action) {
		...
		Log.v("Game", "Marker Count: " + markerCount + " | Max Markers: "
				+ maxMarkers);
		// Do a victory evaluation
	...
	/**
	 * This method should be called from the Game's TimeoutClock to tell the
	 * Game that it should end because the time is up.
	 */
	 ...
	 * 
	 * @param message
	 *            The message to send to those observers.
	 */
	...
	/**
	 * Executes a given GameAction. If successful, it will add it to the game
	 * history.
	 * 
	 * @param action
	...
	/**
	 * Checks for a victory along all possible paths by extracting relevant data
	 * from a MoveAction.
	 * 
	 * @param action
	 *            The MoveAction that encapsulates a player's move.
	 * @return True if that player is victorious, false otherwise.
	 */
	private boolean doVictoryEvaluation(MoveAction action) {
		if (rowVictory(action)) {
			return true;
		} else if (columnVictory(action)) {
			return true;
		} else if (diagonalVictoryTLBR(action)) {
			return true;
		} else if (diagonalVictoryBLTR(action)) {
			return true;
		} else {
			return false;
		}
	...
	private boolean rowVictory(MoveAction action) {
		Vector<Vector<Integer>> gameBoardData = action.getBoard()
				.getGameBoard();
		...
		if(action.getBoard().getPlaceMarkerStrategy().getTag().equals("SlideExistingMarker")) {
			SlideExistingMarker strategy = (SlideExistingMarker) action.getBoard().getPlaceMarkerStrategy();
			startRow = strategy.getPreviousOpenSpotRow();
			startColumn = strategy.getPreviousOpenSpotCol();
		}
	...
	private boolean columnVictory(MoveAction action) {
		Vector<Vector<Integer>> gameBoardData = action.getBoard()
				.getGameBoard();
		int gameBoardSize = action.getBoard().getBoardSize();
		int markersNeededToWin = gameBoardSize;
		...
		for (int i = 0; i < gameBoardSize; i++) {
			Vector<Integer> currentRow = gameBoardData.get((startRow + i)
					% gameBoardSize);
	...
	private boolean diagonalVictoryTLBR(MoveAction action) {
		// Check for a victory along the diagonal.
		// A diagonal victory from the top left to the bottom right can only
		// occur if the player has a marker wherever a row is equal to a column,
		// i.e.:
		// [x _ _ _ _]
		// [_ x _ _ _]
		// [_ _ x _ _]
	...
	private boolean diagonalVictoryBLTR(MoveAction action) {
		// A diagonal victory from the bottom left to the top right can only
		// occur if the sum of the row and column positions is one less than the
		// board size.
		Vector<Vector<Integer>> gameBoardData = action.getBoard()
				.getGameBoard();
		int gameBoardSize = action.getBoard().getBoardSize();
		...
		if (startRow + startColumn == gameBoardSize - 1) {
			for (int i = 0; i < gameBoardSize; i++) {
				...
				Log.d("VictoryCheckBRTL", "currentRow = "
						+ currentRowCalculator);
				Vector<Integer> currentRow = gameBoardData
						.get(currentRowCalculator);
				int currentColumn = (startColumn + i) % gameBoardSize;
	 ...
	 * 
	 * @param timer
	 *            The TimeoutClock object to use to keep track of time.
	 */
